...
filtered_signal1 = combined_signal .* cos(2 * pi * f1 * t) * 2;
filtered_signal2 = combined_signal .* cos(2 * pi * f2 * t) * 2;


Fc_lp = 5; % Cutoff frequency for low-pass filter (Hz)
Fc_2p = 10;
order = 20; % Filter order
lp_filter1 = fir1(order, Fc_lp / (Fs/2)); % Normalized cutoff frequency
lp_filter2 = fir1(order, Fc_2p / (Fs/2));
demodulated_msg1 = filter(lp_filter1, 1, filtered_signal1);
demodulated_msg2 = filter(lp_filter2, 1, filtered_signal2);

figure;
subplot(3,1,1);
plot(t, combined_signal);
title('Received Combined FDMA Signal');
xlabel('Time (s)');
ylabel('Amplitude');
grid on;

subplot(3,1,2);
plot(t, demodulated_msg1);
title('Demodulated User 1 Message Signal');
xlabel('Time (s)');
ylabel('Amplitude');
grid on;

subplot(3,1,3);
plot(t, demodulated_msg2);
title('Demodulated User 2 Message Signal');
xlabel('Time (s)');
ylabel('Amplitude');
grid on;
